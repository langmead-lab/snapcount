...
```{r 'ts', eval = FALSE}
groupA <- SnaptronQueryBuilder$new()
groupA$compilation("gtex")$regions("chr4:20763023-20763023")
groupA$range_filters(strand == "-")
groupA$coordinate_modifier(Coordinates$EndIsExactOrWithin)

groupB <- SnaptronQueryBuilder$new()
groupB$compilation("gtex")$regions("chr4:20763098-20763098")
groupB$range_filters(strand == "-")
groupB$coordinate_modifier(Coordinates$StartIsExactOrWithin)
...
## Junction Union (Merge)

This query provides the ability to get a single RangeSummarizedExpression object of non-redundant junctions across multiple compilations which share a common reference (e.g. HG38).
...
gtex_tcga_jx<-junction_union(gtex,tcga)
head(gtex_tcga_jx)
```
