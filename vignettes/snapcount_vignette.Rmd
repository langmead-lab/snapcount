...
`r Biocpkg('recount')` is a `R` package available via the [Bioconductor](http://bioconductor/packages/recount) repository for packages. 
`R` can be installed on any operating system from [CRAN](https://cran.r-project.org/) after which you can install `r Biocpkg('recount')` 
by using the following commands in your `R` session:

```{r 'installDer', eval = FALSE}
#Snapcount is not yet in BioConfuctor, temporarily, 
...
If getting the counts for *all* the genes, exons, and/or splice junctions in an annotation from a particular study is your goal, then we suggest you use the `r Biocpkg('recount')` package instead of `r Biocpkg('snapcount')`.  
...
`r Biocpkg('snapcount')` can be used to summarize the raw results of the basic queries 
...
`r Biocpkg('snapcount')`'s version of PSI is currently limited to the common case 
...
Additionally the value of -1 is used in `r Biocpkg('snapcount')`'s PSI function to represent 
